<template>
  <div
    id="canvasContainer"
    :class="{
      'add-mode': isAddingElements,
      'panning-mode': isPanningMode,
      'pannable-mode': isPannableMode
    }"
  >
...
export default {
  ...
  methods: {
    ...
    onMouseMove(event) {
      if (this.isPanning) {
        this.paper.translate(event.offsetX - this.panX, event.offsetY - this.panY)
      }
    },
    onJsonnetsEvent(event, payload) {
      if (event === EVENT_ADD_PLACE) {
        ...
        if (payload.isExample) {
          // set request
          this.graph
            .getCell('167d54d6-3a73-40f7-b317-0aa3580a44ac')
            .set('position', { x: 0, y: 144 })
          // set student
          this.graph
            .getCell('48b440c6-43fc-4df6-b874-f758137e90e5')
            .set('position', { x: 48, y: -12 })
          // set review
          this.graph
            .getCell('1952db8b-764c-45da-b2d4-8b1537400377')
            .set('position', { x: 144, y: 144 })
          // set decision
          this.graph
            .getCell('9a172fc4-04cd-49d1-94f3-b8b62d2aed42')
            .set('position', { x: 348, y: 144 })
          // set lecture
          this.graph
            .getCell('846b0b51-9981-40ad-a36a-5ee873f4de5a')
            .set('position', { x: 288, y: -12 })
          // set accept
          this.graph
            .getCell('0e1c227f-c03b-4969-be2b-9b151898a35c')
            .set('position', { x: 504, y: 48 })
          // set reject
          this.graph
            .getCell('82ca126e-1b63-40ad-9f92-d156da6823b8')
            .set('position', { x: 504, y: 240 })
          // set notification
          this.graph
            .getCell('daa76eb3-c88c-4f29-9903-43f2892d70da')
            .set('position', { x: 708, y: 144 })
          // set grade
          this.graph
            .getCell('e6caf7c5-fe05-47fa-8179-cf3b7b3cad2a')
            .set('position', { x: 528, y: -96 })
    ...
    onViewHoverStart(view) {
      if (this.uiState.mode === MODE_CONNECT_START || this.uiState.mode === MODE_MOVE) {
        view.showTools()
      }
    },
    onViewHoverEnd(view) {
