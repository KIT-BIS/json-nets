				...
				<div class="columns scoped-scrollable-outer">
					<div class="column is-one-quarter scoped-scroll-container">
						<aside class="menu is-small scoped-scrollable">
							...
							<ul class="menu-list">
								<li>
									<a @click="() => { toggleActive('input') }">
									Input ({{ transitionsStore.inputArcs.length }} places)
									</a>
							...
							<div class="field">
                              ...
                              " />
								...
								<div class="control is-small jsn-code">
									<Codemirror v-if="uiStateStore.showEditor === 'guard'"
										placeholder="Enter guard expression ..." style="height: 200px"
										:indent-with-tab="true" :tab-size="2" :extensions="extensions"
										v-model="transitionsStore.transition.guard" @ready="onEditorReady"
										:disabled="netStore.transitionTypes[transitionsStore.transition.id] !== 'custom'" />
									<Codemirror v-if="uiStateStore.showEditor === 'preface'"
										placeholder="Enter preface ..." style="height: 200px" :indent-with-tab="true"
										:tab-size="2" :extensions="extensions"
										v-model="transitionsStore.transition.preface" @ready="onEditorReady"
										:disabled="netStore.transitionTypes[transitionsStore.transition.id] !== 'custom'" />
									<Codemirror v-if="uiStateStore.showEditor === 'key'"
										placeholder="Enter output key expression ..." style="height: 200px"
										:indent-with-tab="true" :tab-size="2" :extensions="extensions"
										v-model="transitionsStore.outputArcs[transitionsStore.selectedOutputSnippetIndex].keySnippet"
										@ready="onEditorReady"
										:disabled="netStore.transitionTypes[transitionsStore.transition.id] !== 'custom'" />
									<Codemirror v-if="uiStateStore.showEditor === 'value'"
										placeholder="Enter output value expression ..." style="height: 200px"
										:indent-with-tab="true" :tab-size="2" :extensions="extensions"
						...
						<div class="block" v-if="transitionsStore.assignmentComplete">
							<div v-if="uiStateStore.showEditor == 'guard'" class="field">
								...
								<div v-else-if="!transitionsStore.guardHasError"
									...
									style="height: 200px">
									<span>{{ JSON.stringify(JSON.parse(transitionsStore.guardEvaluation), null, 2)
										}}</span>
								</div>
								<div v-else-if="transitionsStore.guardHasError"
...
/**
 * Provides a special editor with more space to edit transition inscription (preface, guard, output expressions).
 */
